---
title: Cuando un SQL Injection nos puede dejar como un payaso
author: Benjamin
...
# Cuando un SQL Injection nos puede dejar como un payaso

El título suena rudo, pero a decir verdad en muchas empresas y desarrolladores la seguridad la dejan a la ligera. A sí que veamos que podemos hacer con sqlijection.
...
## Blind SQLInjection sobre un entorno controlado Over The Wire
...
Ahora nos dirigimos a la página [natas.natas15.overthewire.com](natas.natas15.overthewire.com) Ingresamos contraseñas y vemos un pequeño input que consulta por un user
si indicamos el usuario del server siguiente nos dice que existe y ponemos otra cosa nos indica que no existe
...
enviando la siguiente información vemos un error en query con la siguiente sentencia
...
Ya tenemos un indicio que es vulnerable nuestro input ¿pero qué podemos hacer? Si lo único respuesta es un "user exists" o "user dont exist"?. Si podemos ejecutar una sentencia definida por nosotros en donde controlamos que la respuesta en forma booleana podremos generar la siguiente injección para descifrar la clave
